...
> 本章面向部署完成的管理员，讲解如何通过后台让系统"喂饱知识、调好状态"。
> 后台共有四个模块：**仪表盘 / 知识库管理 / 系统提示词 / 模型配置**。

### 1. 登录管理后台

1. 打开网站首页，点击**右上角「管理员」按钮**
2. 弹窗中输入账号密码——就是你 `.env` 里配置的 `ADMIN_USERNAME` / `ADMIN_PASSWORD`
3. 登录成功后自动进入后台；以后也可以从左侧会话列表底部的「管理后台」入口直接进入
4. 后台顶栏右侧：「返回对话」回到聊天页；「退出登录」结束管理会话（离开电脑建议退出）

> 登录提示密码错误？回看 [FAQ Q2](#六常见问题-faq)（大概率是密码里用了 `$` 符号）。

### 2. 知识库管理：让 AI 学会你的资料

AI 回答的引用来源全部来自这里上传的文档。**不传文档，AI 只能靠通用知识泛泛而谈。**

**上传文档：**

1. 进入「知识库管理」→ 点击右上角 **「＋ 上传文档入库」**
2. 支持格式：**PDF / Word(docx) / Markdown / TXT / HTML**，可一次多选，单个文件不超过 20MB
3. 上传后立即出现在列表中，状态从左到右流转：

| 状态 | 含义 |
|---|---|
| 排队中 | 已接收，等待处理 |
| 解析中 | 提取文档文字 |
| 清洗中 | LLM 正在整理润色文本(最耗时的一步) |
| 分块中 | 切成检索用的小段 |
| 向量化中 | 生成语义向量写入向量库 |
| **已入库** | ✅ 完成，可以检索了 |
| 失败 | 出错了，红色标注原因 |

4. **耐心等待**：超过 300KB 的文档清洗入库约需 1-3 分钟，页面每几秒自动刷新状态
5. 重复内容自动去重：同一份文档传两遍，第二遍会被跳过

**列表操作三件套：**

- **预览分片**：查看文档被切成了哪些小段——是检查入库质量的好办法（分片应是语义完整的段落，而不是被拦腰截断的句子）
- **重新处理**：修改过模型配置(如更换清洗模型、调整分块大小)后，让旧文档按新配置重新走一遍入库流程；失败的文档也用它重试
- **删除**：彻底移除该文档，**同步删除其向量与分片**，相关的引用会立即失效

**内容更新了怎么办？** 文档内容有修订时，直接上传新版文件即可（内容变了不会被去重拦截）；
若只想让旧文档按新参数重新处理，用「重新处理」；想彻底换掉就「删除」旧文档再传新的。

**验证效果**：回到对话页提问文档里的内容，回答下方应出现「引用来源」卡片。

### 3. 系统提示词：定义 AI 的人设与回答规则

「系统提示词」页面编辑的是 AI 的**总体行为指令**（角色设定、回答风格、约束规则），当前提供 `agent_system` 一个可编辑项。

- 编辑后点击 **「保存并生效」**：写入数据库，**下一轮对话立即生效**，无需重启任何服务
- 点击 **「恢复默认」**：清空自定义内容，回退到代码内置版本（`backend/app/llm/prompts.py`）
- 页面会显示当前状态（已自定义 / 默认值）和最后更新时间

修改建议：

- ✅ 可以调整：回答语气、篇幅偏好、"只依据资料回答"的严格程度、是否主动引导追问
- ⚠️ 谨慎删除：不要删掉"根据检索到的资料回答""标注引用来源"这类约束——删了容易睁眼说瞎话（幻觉）
- 改完发一条测试消息对比效果，不满意就再调或恢复默认

### 4. 模型配置：在线调优 RAG 全流程

「模型配置」页面把 `.env` 里影响回答质量的参数做成了可视化表单，**保存后即时生效**（配置存入数据库并覆盖 .env 默认值，重启后自动重放）。

按分组说明关键参数：

| 分组 | 参数 | 作用与建议 |
|---|---|---|
| DeepSeek | `temperature` | 回答随机性。0.2-0.4 严谨稳定(推荐校园问答)，调高更有创意但易跑偏 |
| DeepSeek | `max_tokens` | 单次回答长度上限，默认 4096 够用 |
| GLM | `embedding_dimensions` | 向量维度 ⚠️ **改了它必须把所有文档重新入库**，否则新旧向量不兼容，检索会乱 |
| GLM | `embedding_batch_size` | 入库批量，一般不动 |
| 检索策略 | `召回数 Top K` | 每次检索取几段资料，越大召回越全但可能引入噪声，默认 8 |
| 检索策略 | `混合检索(BM25+RRF)` | 关键词+语义双路召回，**保持开启** |
| 分块 | `chunk_size` / `chunk_overlap` | 分块大小与相邻重叠，默认 600/120，改动后需重新入库 |
| 会话记忆 | `memory_window_messages` | 带入上下文的历史消息条数 |

**操作要点：**

- 只改动你想改的项，页面会统计修改数量（按钮显示「保存修改 (n)」）
- **API Key 脱敏显示**：已保存的 key 只显示前后几位，没修改就点保存不会弄丢它
- 换新 API Key（比如余额用完了）：直接把新 key 粘贴进对应输入框保存即可，**即时生效，不用重启容器**
- 改动分块/维度类参数后，去「知识库管理」对文档执行「重新处理」才会作用到已有数据

### 5. 仪表盘：掌握运行状态

「仪表盘」是运营数据总览，**每 30 秒自动刷新**，也可手动切换「近 7 天 / 近 30 天」：

| 指标 | 含义 |
|---|---|
| 站点访问量 (PV) | 页面被打开的次数 |
| 问答次数 | 用户一共提了多少个问题 |
| Token 消耗 | 分输入(prompt)/输出(completion)/合计，是 API 账单的直接来源 |
| 平均响应时间 | 从提问到回答完成的平均耗时 |
| 会话总数 / 知识库文档 / 向量分片总数 | 知识库规模一览 |

参考：一次带文档引用的问答约消耗数千 Token；展示期若担心 API 余额，
可在此观察消耗速度，并到对应平台提前充值。

---

### 6. 推荐的上手顺序（第一次部署照着做）

1. 登录后台 → 「知识库管理」→ 上传 3-5 份核心资料（如学校介绍、办事指南）→ 等全部「已入库」
2. 回到对话页逐条提问资料中的内容，确认引用来源正确
3. 「系统提示词」按需微调人设 → 保存 → 再提问对比
4. 「模型配置」如需调整只动 `temperature` 和 `Top K`，小步验证
5. 「仪表盘」观察 Token 消耗速度，规划 API 充值节奏
6. 正式展示前：删除测试会话（对话页侧栏可删），保持界面清爽
